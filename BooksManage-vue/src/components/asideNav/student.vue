  ...
  <el-menu router class="el-menu-vertical-demo" :default-active="$route.path">
    <el-menu-item v-for="item in menuData" :key="item.name" :index="item.path">
      ...
      <template v-slot:title>
        <span class="right-align">{{ item.label }}</span>
      </template>
    </el-menu-item>
  </el-menu>
</template>
...
const menuData = ref([
  {
    path: `/student/${student_id.value}/searchBook`,
    name: "searchBooks",
    label: "图书查询",
    icon: "Search",
  },
  {
    path: `/student/${student_id.value}/readInformation`,
